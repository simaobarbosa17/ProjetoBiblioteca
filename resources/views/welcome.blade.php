...
<body class="bg-[#FDFDFC] text-[#1b1b18] flex p-6 lg:p-8 items-center justify-center min-h-screen flex-col">
    <header class="w-full lg:max-w-4xl max-w-[335px] text-sm mb-6 hidden lg:block">
        @if (Route::has('login'))
            <nav class="flex items-center justify-center gap-6">
                @auth
                    @if (Auth::user()->role === 'admin')
                        <a href="{{ url('/admin/dashboard') }}"
                            class="inline-block px-8 py-3 border border-[#19140035] hover:border-[#1915014a] text-[#1b1b18] rounded-md text-lg font-medium leading-normal transition-all duration-200 hover:bg-[#f5f5f3]">
                            Dashboard Admin
                        </a>
                    @else
