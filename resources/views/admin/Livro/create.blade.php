    ...
    <div class="py-8">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-base-100 shadow-md sm:rounded-lg p-8">
                <form action="{{ route('admin.livros.store') }}" method="POST" enctype="multipart/form-data" novalidate>
                    @csrf
                    ...
                    <div class="form-control w-full mt-6">
                        ...
                        >
                            @foreach (\App\Models\Autores::all() as $autor)
                                <option
                                    value="{{ $autor->id }}"
                                    {{ collect(old('autor_ids'))->contains($autor->id) ? 'selected' : '' }}
                                >
                                    {{ $autor->nome }}
                                </option>
                            @endforeach
                    ...
                    <div class="form-control w-full mt-6">
                        <label for="capa" class="label">
                            <span class="label-text font-semibold text-base-content">Capa</span>
                        </label>
                        <input
                            id="capa"
                            name="capa"
